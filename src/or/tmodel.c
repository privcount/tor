  ...
  if (tmodel->trans_prob) {
    ...
    for (int i = 0; i < tmodel->num_states; i++) {
      if (tmodel->trans_prob[i]) {
        for (int j = 0; j < tmodel->num_states; j++) {
          log_info(LD_GENERAL, "found trans_prob[%i][%i] '%f'", i, j, tmodel->trans_prob[i]);
        }
      }
    }
  }
  ...
  if (tmodel->emit_dp && tmodel->emit_mu && tmodel->emit_sigma) {
    ...
    for (int i = 0; i < tmodel->num_states; i++) {
      if (tmodel->emit_dp[i] && tmodel->emit_mu[i] && tmodel->emit_sigma[i]) {
        for (int j = 0; j < tmodel->num_obs; j++) {
          log_info(LD_GENERAL, "found emit_dp[%i][%i] '%f'", i, j, tmodel->emit_dp[i]);
          log_info(LD_GENERAL, "found emit_mu[%i][%i] '%f'", i, j, tmodel->emit_mu[i]);
          log_info(LD_GENERAL, "found emit_sigma[%i][%i] '%f'", i, j, tmodel->emit_sigma[i]);
        }
      }
    }
  }
